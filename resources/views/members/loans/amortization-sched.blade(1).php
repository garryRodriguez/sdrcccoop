        ...
        @forelse ($amort_data as $amortization_data)
            ...
            <table class="table table-sm table-hover align-middle bg-white border text-secondary w-75 mx-auto shadow">
                ...
                {{-- <tbody class="text-center"> --}}
                    ...
                    <!--        <tr class="text-center">-->
                    <!--            <td>{{$i}}</td>-->
                    <!--            <td>{{round($amortization_data->approved_loan_amount / 24), 2}}</td>-->
                    <!--            <td>{{$amortization_data->interest / 24}}</td>-->
                    <!--            <td>{{round(($amortization_data->approved_loan_amount + $amortization_data->interest) / 24, 2)}}</td>-->
                    <!--            <td></td>-->
                    <!--        </tr>-->
                    <!--    @endfor-->
                    <!--<tr class="mt-3 text-center">-->
                    <!--    <td class="fw-bold text-danger">Total: </td>-->
                    <!--    <td class="fw-bold text-danger">{{$amortization_data->approved_loan_amount}}</td>-->
                    <!--    <td class="fw-bold text-danger">{{($amortization_data->interest / 24) * 24}}</td>-->
                    <!--    <td class="fw-bold text-danger">{{round(($amortization_data->approved_loan_amount + $amortization_data->interest) / 24, 2) * 24}}</td>-->
                    <!--    <td></td>-->
                    <!--</tr>-->
                    ...
                    @for ($i = 1; $i <= $amortization_data->term_of_loan * 2; $i++)
                            <tr class="text-center">
                                <td>{{$i}}</td>
                                <td>{{round($amortization_data->approved_loan_amount / $amortization_data->term_of_loan)  / 2, 2}}</td>
                                <td>{{$amortization_data->interest / $amortization_data->term_of_loan / 2}}</td>
                                <td>{{round(($amortization_data->approved_loan_amount + $amortization_data->interest) / $amortization_data->term_of_loan / 2, 2)}}</td>
                                <td></td>
                            </tr>
                        @endfor
                    <tr class="mt-3 text-center">
                        <td class="fw-bold text-danger">Total: </td>
                        <td class="fw-bold text-danger">{{$amortization_data->approved_loan_amount}}</td>
                        <td class="fw-bold text-danger">{{($amortization_data->interest / $amortization_data->term_of_loan) * $amortization_data->term_of_loan}}</td>
                        <td class="fw-bold text-danger">{{round(($amortization_data->approved_loan_amount + $amortization_data->interest) / $amortization_data->term_of_loan, 2) * $amortization_data->term_of_loan}}</td>
                        <td></td>
                    </tr>
